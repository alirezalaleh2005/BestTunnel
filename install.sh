#!/bin/bash
#=================================================
# Paqet-X Client Manager v1.0 (License Removed Version)
#=================================================
# Colors
...
# --- License URLs REMOVED --- 
# No longer needed as the system runs without license checks
...
echo "║                                                              ║"
echo "║   GitHub  : github.com/iliya-Developer                       ║"
echo "║   Telegram: t.me/iliyadevsh                                  ║"
echo "║                                                              ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"
}
...
local name="$1"
# License URL argument removed. Service now runs standalone.
...
# ─── Download Source Selection (Removed License Server Logic) ───
# Since license is removed, we just need a download source.
# Assuming you have a single reliable source or want to use the old Kharej source for binaries only.
echo -e "${CYAN}Select Download Server:${NC}"
echo -e "────────────────────────────────────────────────────────────────"
echo -e " ${GREEN}[1]${NC} Iran   (iran.iliyadev.ir)"
echo -e " ${GREEN}[2]${NC} Kharej (kharej.iliyadev.ir)"
echo ""
read -p "Choice [1-2] (default: 2): " srv_choice </dev/tty
srv_choice="${srv_choice:-2}"

local download_base=""
case $srv_choice in
1) download_base="http://iran.iliyadev.ir:8080" ;;
*) download_base="http://kharej.iliyadev.ir:8080" ;;
esac

echo -e " ${GREEN}[+]${NC} Selected: ${CYAN}$download_base${NC}"
echo ""

local download_url="${download_base}/api/download?arch=${arch}"
...
# Call create_systemd_service with only the name (no license URL)
